systemctl start mongod

sleep 5

cd /home/cpswt/webgme-engine

node src/bin/import.js -m mongodb://127.0.0.1:27017/c2webgme -p HelloWorldOmnetpp ../cpswt-meta/seeds/_archive/HelloWorldOmnetpp.webgmex

cd /home/cpswt/cpswt-meta

# generate .zip file
nvm use 8.10.0
export PYTHONPATH=$PWD/src/plugins/PyFederatesExporter/PyFederatesExporter
python3 $PWD/src/plugins/PyFederatesExporter/run_debug.py

#unzip .zip
cd /tmp
unzip HelloWorldOmnetpp.zip
cd HelloWorldOmnetpp

# # run experiment
# export JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-arm64
# export PATH=$JAVA_HOME/bin:$PATH

# gradle wrapper --gradle-version=7.4


